...
async function runTests() {
  try {
    ...
    const aadharNumber = '11111111' + Math.random().toString().substring(2, 6); // Unique Aadhar
    const citizenUser = {
    ...
    console.log('  📋 Full response:', JSON.stringify(registerCitizenRes.data, null, 2));
    
    // Extract citizen ID - handle different response structures
    let citizenId;
    if (registerCitizenRes.data.data.user._id) {
      citizenId = registerCitizenRes.data.data.user._id;
    } else if (registerCitizenRes.data.data.user.id) {
      citizenId = registerCitizenRes.data.data.user.id;
    } else {
